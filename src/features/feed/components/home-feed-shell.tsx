...
export function HomeFeedShell({ activeType }: HomeFeedShellProps) {
  ...
  useEffect(() => {
    function handleWindowWheel(event: WheelEvent) {
      ...
      if (!feedElement || event.defaultPrevented || event.ctrlKey || event.deltaY === 0) {
        return;
      }
      ...
      const deltaY = getWheelDelta(event, feedElement.clientHeight);
      const nextScrollTop = Math.min(maxScrollTop, Math.max(0, feedElement.scrollTop + deltaY));

      if (nextScrollTop === feedElement.scrollTop) {
        return;
      }
  ...
  return (
    <div className="flex h-[calc(100dvh-8rem)] flex-col md:h-screen">
      ...
      <div className="min-h-0 flex-1 px-3 pt-3 md:px-4">
        <section
